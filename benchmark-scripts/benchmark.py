...
def parse_args(print=False):
    ...
    #parser.add_argument('--performance_mode', default="powersave", help='the system performance setting [powersave | performance]')
    ...
    parser.add_argument('--results_dir', default=os.path.join(os.curdir, 'results'), help='full path to the desired directory for logs and results')
    parser.add_argument('--duration', type=int, default=30, help='time in seconds, not needed when --stream_density is specified')
    ...
    parser.add_argument('--target_device', default='CPU', help='desired running platform [cpu|core|xeon|dgpu.x]')
    parser.add_argument('--compose_file', default=None, action='append', help='full path to the pipeline compose file(s) "/path/to/pipeline-compose.yml". can be used multiple times')
    parser.add_argument('--retail_use_case_root', default=os.path.join(os.curdir, '..', '..', 'retail-use-cases'), help='full path to the retail-use-cases repo root')
    if print:
        parser.print_help()
        return
